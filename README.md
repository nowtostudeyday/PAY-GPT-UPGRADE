...
| 本地 Stripe 流程 | 套餐双重校验 | 按 CDK 绑定的 Plus / Pro 5x / Pro 20x 套餐，在填卡前校验 Checkout 页面展示的目标套餐；支付后再次读取 OpenAI 订阅原始套餐标识。任一阶段无法精确匹配即不报告成功。 |
| 账单与任务状态 | 错充保护 | 扣款后套餐不一致时，账单标为“需人工核验”，任务不判成功且 CDK 保留，避免用户重新提交导致重复扣款；账单仅在后置套餐校验通过后标记成功。 |
